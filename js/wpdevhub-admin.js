...
jQuery( document ).ready( function( $ ) {
    ...
    jQuery( "a.add_media" ).on( "click", function() {
        wp.media.model.settings.post.id = DSCF_DLM_Admin.wp_media_post_id;
    });

    try{
        jQuery(".DSCF_DLM_jquery-ui-sortable").sortable();
        jQuery(".DSCF_DLM_jquery-date-picker").datepicker();
        jQuery(".DSCF_DLM_jquery-datetime-picker").datetimepicker();
    }catch(e){}

    // Load User Messages
    DSCF_DLM_UserMessages.load();
...
var DSCF_DLM_Admin = {
    ...
    openMediaLibrary:function( objectName , set_to_post_id ){
        ...
        // Store the Old ID
        DSCF_DLM_Admin.wp_media_post_id = wp.media.model.settings.post.id;
    ...
    openMediaLibraryMultiple:function( objectName ){
        ...
        file_frame.on( "select", function() {
            ...
            selection.map( function( attachment ) {
                ...
                var url_images = DSCF_DLM_vars.url+"/images/";
                var wrapperId = objectName+"_wrapper_"+attachment.id;

                // Add the preview
                var html = '<div id="'+wrapperId+'" class="dlm-ml-thumb-wrapper">' +
                    '<div class="dlm-align-right"><img src="'+url_images+'/cancel.png" style="width:14px;" onclick="jQuery(\'#'+wrapperId+'\').remove()" /></div>' +
    ...
    openMediaLibrarySmdMultiple:function( objectName ){
        ...
        file_frame.on( "select", function() {
            ...
            selection.map( function( attachment ) {
                ...
                // Add the preview
                /*
                var html = '<li id="'+wrapperId+'" class="dlm-ml-row">' +
                    '<div class="dlm-align-right"><img src="'+url_images+'/cancel.png" style="width:14px;" onclick="jQuery(\'#'+wrapperId+'\').remove()" /></div>' +
                    '<div class="dlm-inline-block"><img src="'+attachment.url+'" width="100" height="100"></div>' +
                ...
                */
                ...
                var html = '<li id="'+wrapperId+'">' +
                    '<table id="'+objectName+'_table" class="dlm-table dlm-table-striped dlm-table-padding-mini smdDiveReportTable">' +
                    '<tr>' +
                    '<th>Image Title</th>' +
                    '<td><input type="text" name="'+objectName+'_title_'+attachment.id+'" size="50" /></td>' +
                    '<td class="dlm-align-right"><button type=button onclick="jQuery(\'#'+wrapperId+'\').remove();">delete</button></td>' +
                    '</tr>' +
                    '<tr>' +
                    '<th>Camera Settings</th>' +
                    '<td><input type="text" name="'+objectName+'_cameraSettings_'+attachment.id+'" size="50" /></td>' +
                    '<td rowspan="2" class="dlm-align-center"><img src="'+attachment.url+'" width="150" height="150" /></td>' +
                    '</tr>' +
                    '<tr>' +
                    '<th>Comments</th>' +
                    '<td><textarea class="dlm-textarea" style="height:100px;" name="'+objectName+'_caption_'+attachment.id+'" id="'+objectName+'_caption_'+attachment.id+'"></textarea></td>' +
                    '</tr>' +
                    '</table>' +
                    '<input type="hidden" name="'+objectName+'_smdMediaId_'+attachment.id+'" id="'+objectName+'_smdMediaId_'+attachment.id+'" value="'+attachment.id+'" />' +
...
/*
    addSmdDiveReport:function(editorKeyname){
        ...
        '<table id="'+keynamePrefix+'_table" class="dlm-table dlm-table-striped dlm-table-padding-mini smdDiveReportTable">' +
        '<tr>' +
        '<th class="dlm-align-right">Dive Title</th>' +
        '<td colspan="4"><input type="text" name="'+keynamePrefix+'_title" size="50" /></td>' +
        '<td class="dlm-align-right"><button type=button onclick="DSCF_DLM_Admin.deleteSmdDiveReport(\''+keynamePrefix+'_wrapper\')">delete</button></td>' +
        '</tr>' +
        '<tr>' +
        '<th class="dlm-align-right">Dive Rating</th>' +
        '<td><input type="text" name="'+keynamePrefix+'_diveRating" size="10" /></td>' +
        '<th class="dlm-align-right">Dive Depth</th>' +
        '<td><input type="text" name="'+keynamePrefix+'_diveDepth" size="10" /></td>' +
        '<th class="dlm-align-right">Exposure Protection</th>' +
        '<td><input type="text" name="'+keynamePrefix+'_exposureProtectionId" size="10" /></td>' +
        '</tr>' +
        '<tr>' +
        '<th class="dlm-align-right">Visibility</th>' +
        '<td><input type="text" name="'+keynamePrefix+'_visibility" size="10" /></td>' +
        '<th class="dlm-align-right">Bottom Time</th>' +
        '<td><input type="text" name="'+keynamePrefix+'_bottomTime" size="10" /></td>' +
        '<th class="dlm-align-right">Weight Used</th>' +
        '<td><input type="text" name="'+keynamePrefix+'_weight" size="10" /></td>' +
        '</tr>' +
        '<tr>' +
        '<th class="dlm-align-right">Surface Temp</th>' +
        '<td><input type="text" name="'+keynamePrefix+'_temperatureSurface" size="10" /></td>' +
        '<th class="dlm-align-right">Tank PSI Start</th>' +
        '<td><input type="text" name="'+keynamePrefix+'_tankPsiStart" size="10" /></td>' +
        '<td></td>' +
        '<td></td>' +
        '</tr>' +
        '<tr>' +
        '<th class="dlm-align-right">Bottom Temp</th>' +
        '<td><input type="text" name="'+keynamePrefix+'_temperatureBottom" size="10" /></td>' +
        '<th class="dlm-align-right">Tank PSI End</th>' +
        '<td><input type="text" name="'+keynamePrefix+'_tankPsiEnd" size="10" /></td>' +
        '<td></td>' +
        '<td></td>' +
        '</tr>' +
        '<tr>' +
        '<th class="dlm-align-right">Comments</th>' +
        '<td colspan="5"><textarea name="'+keynamePrefix+'_comments" class="dlm-textarea"></textarea></td>' +
        '</tr>' +
        '</table>' +
...
*/
    addBlankRow:function(objectName, beforeId, includeCheckbox){
        ...
        html += '<input type="text" name="'+textObjectName+'" value="" size="50" /><button type=button onclick="DSCF_DLM_Admin.deleteOptionRow(\''+textObjectName+'\')">delete</button>';
        html += '</div>';
    ...
    addDttListRow:function(objectName, beforeId){
        ...
        html += '<button onclick="DSCF_DLM_Admin.openMediaLibrary(\''+rowKey+'_mediaId\', 0)">Open Media Library</button>';
        html += '<input type="hidden" name="'+rowKey+'_mediaId" id="'+rowKey+'_mediaId" value="0">';
        html += '<a href="javascript:DSCF_DLM_Admin.removeDttImage(\''+rowKey+'\');">remove</a>';
        html += '</div>';
    ...
    addWdhPluginFeatureRow:function(objectName, beforeId){
        ...
        html += '<div id="'+rowKey+'_wrapper" class="dlm-pbox5">';
        html += '<div>';
        html += '<button style="float:right;" onClick="javascript:DSCF_DLM_Admin.deleteGenericWrapperRow(\''+rowKey+'\');">Delete Feature</button>';
        html += '<input type="hidden" name="'+rowId+'" value="'+counter+'" />';
        html += 'Title: <input type="text" name="'+rowKey+'_title" value="" size="50" />';
        html += '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Primary Image: ';
        html += '<img id="'+rowKey+'_mediaId_preview" src="'+placeholder+'" style="width:50px; height:50px; padding:10px; vertical-align:middle;" />';
        html += '<button onclick="DSCF_DLM_Admin.openMediaLibrary(\''+rowKey+'_mediaId\', 0)">Open Media Library</button>';
        html += '<input type="hidden" name="'+rowKey+'_mediaId" id="'+rowKey+'_mediaId" value="0">';
        html += '<a href="javascript:DSCF_DLM_Admin.removeWdhPluginFeatureImage(\''+rowKey+'\');">remove</a>';
        html += '</div>';
    ...
    addDrcIngredientRow:function(){
        ...
        var measurements = DSCF_DLM_extra_vars.measurements;
    ...
    createSlug:function(slugElemId, sourceElemId, slugClassName){
        ...
        setTimeout(DSCF_DLM_Admin.validateSlug(slugElemId, slugClassName), 100);
    },

    validateSlug:function(slugElemId, slugClassName){
        //Get the proposed value
        var newSlug = jQuery('#slug_'+slugClassName+'_preview').val();
        jQuery('#slug_'+slugClassName+'_previewText').html('<img src="'+DSCF_DLM_vars.url+'/images/loading2.gif" style="vertical-align:top; width:16px;" />');
        ...
        var queryParams = {
            ...
            slug_text: newSlug,
            elem_id: slugElemId,
            action: 'WPDEVHUB_CONST_DLM_SLUG-validate-slug',
            object_id: objectId
        };

        //console.log("URL: "+DSCF_DLM_vars.url_ajax);
        //console.dir(queryParams);

        jQuery.post(DSCF_DLM_vars.url_ajax, queryParams, function( response ){
            //console.dir(response);
            jQuery('#'+response.elemId).val(response.slug);
            jQuery('#slug_'+response.class_name+'_preview').val(response.slug);
            jQuery('#slug_'+response.class_name+'_previewText').html('<img src="'+DSCF_DLM_vars.url+'/images/accept.png" style="width:16px;" />');
            setTimeout(function(){
    ...
    refreshSwatch:function(elemId) {
        ...
        try{
            ...
            var hex = DSCF_DLM_Admin.hexFromRGB( red, green, blue );
    ...
    inputSwatchValue:function(elemId){
        ...
        hex = hex.replace("#", "");
        jQuery( "#"+elemId+"_swatch_value").attr( "value", hex );
        var result = DSCF_DLM_Admin.hexToRgb(hex);
...
var DSCF_DLM_UserMessages = {
    counter:0,
    load: function(){
        var html = jQuery("#dlm-user-messages-tmp").html();
        if(html != undefined){
            jQuery("#dlm-wrapper").prepend(html);
            jQuery("#dlm-user-messages-tmp").remove();
            setTimeout(function(){
                jQuery(".dlm-user-message").fadeOut();
            }, 5000);
        }
    },
    remove: function(id){
...
var DSCF_DLM_ChartObject = function(){
    var elemId = null;
...
var DSCF_DLM_Charts = {
    ...
    loadGoogle:function() {
        if(DSCF_DLM_Charts.isGoogleLoaded){
            DSCF_DLM_Charts.loadGoogleVisualization();
        }else{
            DSCF_DLM_Charts.loadJsFile("https://www.google.com/jsapi?callback=DSCF_DLM_Charts.loadGoogleVisualization");
        }
    },
    ...
    loadGoogleVisualization:function() {
        ...
        if(google == undefined){
            DSCF_DLM_Charts.loadGoogle();
            return; //Do the main loader
        }else{
            DSCF_DLM_Charts.isGoogleLoaded = true;
        }

        //Load the visualizer
        if(DSCF_DLM_Charts.isGoogleVisLoaded){
            DSCF_DLM_Charts.drawGoogleCharts();
        }else{
            google.load("visualization", "1", {'packages':['corechart','table','annotationchart'], "callback" : DSCF_DLM_Charts.drawGoogleCharts});
        }
    },
    ...
    drawGoogleCharts:function(){
        ...
        if(DSCF_DLM_Charts.isGoogleLoaded){
            if(google.visualization == undefined){
                ...
                DSCF_DLM_Charts.isGoogleVisLoaded = true;
            }
        }else{
            DSCF_DLM_Charts.loadGoogle();
        ...
        console.dir(DSCF_DLM_Charts.chartsToDo);

        // Loop through any elements added to our to-do array...
        while(DSCF_DLM_Charts.chartsToDo.length > 0){
            var dimbalChart = DSCF_DLM_Charts.chartsToDo[0];
            ...
            if(dimbalChart.elemId != undefined){
                ...
                if(chartElement != undefined){
                    ...
                    if(dimbalChart.data != undefined && dimbalChart.data.length > 1){
                        ...
                        DSCF_DLM_Charts.removeArrayEntry(DSCF_DLM_Charts.chartsToDo, 0);
                    ...
                    /*  Assignments vary depending on Chart Type - so skipping for now
                     ...
                     var randomColor = DSCF_DLM_Charts.getRandomColor();
