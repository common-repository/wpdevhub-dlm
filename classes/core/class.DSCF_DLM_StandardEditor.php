...
class DSCF_DLM_StandardEditor{
    ...
    const ET_DATE_TIME = 25;        // In order to enable Date Time it has to be included
        //wp_enqueue_script( WPDEVHUB_CONST_DLM_SLUG.'jquery-time-picker' ,  WPDEVHUB_CONST_DLM_URL.'/js/jquery-ui-timepicker-addon.min.js',  array('jquery' ));
    const ET_SKIP = 26;
    ...
    public static function buildPageTemplate($class, $editorParams=array(), $forceEditor=false){
        ...
        //DSCF_DLM_Utilities::logMessage("Request Vars: ".print_r($_REQUEST, true));
        ...
        if($object && isset($_REQUEST['formEditor'])){
            ...
            if($forceEditor){
                ...
                $html .= DSCF_DLM_StandardEditor::buildEditor($options, '#');
            }else{
                unset($_REQUEST['id']);
            }
        }else{
    ...
    public static function buildEditor($options, $target, $id="", $forceDemoOnly=false){
        ...
        if(empty($tabRows)){
            $html .= '<table class="dlm-content-editor">';
            foreach($options as $optionRow){
                if($forceDemoOnly){
            ...
            foreach($options as $optionRow){
                if(array_key_exists('rowType', $optionRow) && $optionRow['rowType']=='tabHeader'){
                    ...
                    $html .= '<table class="styledTable1 dlm-content-editor">';
                    $firstDiv = false;
                }else{
                    $html .= self::buildEditorRow($optionRow);
                }
    ...
    public static function buildEditorRow($options=array()){
        ...
        if($options['formType'] == self::ET_HIDDEN){
            ...
            $html .= '<tr>';
            $html .= '<td>';
            $html .= '<div class="dlm-content-editor-title">'.$options['title'].'</div>';
            $html .= '<div class="dlm-content-editor-help">'.$options['help'].'</div>';
            $html .= '</td>';
            $html .= '<td>';
    ...
    public static function buildEditorCell($options){
        ...
        switch ($options['formType']){
            case self::ET_TEXT:
                ...
                //DSCF_DLM_Utilities::logMessage("Common Editor TEXT: Value:".print_r($options['value'],true));
                break;
            ...
            case self::ET_SLUG_EDITOR:
                $html .= '
					...
					<br /><a href="javascript:DSCF_DLM_Admin.createSlug(\''.$options['objectName'].'\',\''.$options['sourceName'].'\',\''.$options['slugClassName'].'\');">create from title</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:DSCF_DLM_Admin.validateSlug(\''.$options['objectName'].'\',\''.$options['slugClassName'].'\');">validate slug</a>';
                break;
            case self::ET_TEXT_ADDITIONAL:
                ...
                if(DSCF_DLM_Utilities::validateArray($options['value'])){
                    foreach($options['value'] as $key=>$value){
                        $html .= '<div id="'.$options['objectName'].$key.'_wrapper"><input type="text" id="'.$options['objectName'].$key.'" name="'.$options['objectName'].$key.'" value="'.$value.'" size="'.$size.'"  /><button type=button onclick="DSCF_DLM_Admin.deleteOptionRow(\''.$options['objectName'].$key.'\');">delete</button></div>';
                    }
                }
                $html .= '<div id="addMoreRows'.$options['objectName'].'"><a href="javascript:DSCF_DLM_Admin.addBlankRow(\'moreRows'.$options['objectName'].'\',\'addMoreRows'.$options['objectName'].'\');">add row</a></div>';
                break;
            case self::ET_TEXT_MENU_ADDITIONAL:
                ...
                if(DSCF_DLM_Utilities::validateArray($options['value'])){
                    foreach($options['value'] as $key=>$value){
                        ...
                        $html .= '</select>';
                        $html .= '<button type=button onclick="DSCF_DLM_Admin.deleteOptionRow(\''.$options['objectName'].$key.'\');">delete</button></div>';
                    }
                }
                $html .= '<div id="addMoreRows'.$options['objectName'].'"><a href="javascript:DSCF_DLM_Admin.addBlankRow(\'moreRows'.$options['objectName'].'\',\'addMoreRows'.$options['objectName'].'\');">add row</a></div>';
                break;
            ...
            case self::ET_MENU_STATUS:
                $html .= '<select id="'.$options['objectName'].'" name="'.$options['objectName'].'"> <label for="'.$options['objectName'].'"></label>';
                $html .= '<option value="">--</option>';
                foreach(DSCF_DLM_StandardObjectRecord::getAllStatusMarks() as $key=>$value){
                    $selected = '';
                    if($key==$options['value']){ $selected=' selected="selected"'; }
            ...
            case self::ET_MENU_USERID_USERNAME:
                $userArray = DSCF_DLM_Utilities::getUserLoginsAsMenuArray();
                $html .= '<select id="'.$options['objectName'].'" name="'.$options['objectName'].'"> <label for="'.$options['objectName'].'"></label>';
                $html .= '<option value="">--</option>';
                ...
                break;
            case self::ET_DATE:
                $html .= '<input type="text" id="'.$options['objectName'].'" class="DSCF_DLM_jquery-date-picker" name="'.$options['objectName'].'" value="'.DSCF_DLM_StandardObjectRecord::formatJQueryDate($options['value']).'" />';
                $html .= ' '.self::renderAjaxInsertTodaysDate($options['objectName']).' <label for="'.$options['objectName'].'"></label>';
                break;
            case self::ET_DATE_TIME:
                $html .= '<input type="text" id="'.$options['objectName'].'" class="DSCF_DLM_jquery-datetime-picker" name="'.$options['objectName'].'" value="'.DSCF_DLM_StandardObjectRecord::formatJQueryDateTime($options['value']).'" />';
                $html .= ' '.self::renderAjaxInsertTodaysDate($options['objectName']).' <label for="'.$options['objectName'].'"></label>';
                break;
            ...
            case self::ET_DPM_ANSWER_CHOICE_PICKER:
                ...
                foreach($options['formOptions'] as $key=>$value){
                    $checked = '';
                    if($value->status == DSCF_DLM_StandardObjectRecord::STATUS_ACTIVE){ $checked=' checked="checked"'; }
            ...
            case self::ET_DQQ_ANSWER_CHOICE_PICKER:
                ...
                foreach($options['value'] as $key=>$value){
                    ...
                    $html .= '<div id="'.$options['objectName'].$key.'_wrapper"><input type="radio" id="'.$options['objectName'].$key.'_rdo" name="'.$options['objectName'].'_rdo"'.$checked.' value="'.$key.'"><input type="text" name="'.$options['objectName'].$key.'_txt" value="'.$value.'" size="'.$size.'" /> <input type="button" name="'.$options['objectName'].$key.'_dlt" value="Delete" onClick="javascript:DSCF_DLM_Quiz.deleteOptionRow(\''.$options['objectName'].$key.'\')" /></div>';
                }
                $html .= '<div id="addMoreQuizRows'.$options['objectName'].'"><a href="javascript:DSCF_DLM_Quiz.setCounter('.$key.');DSCF_DLM_Quiz.addBlankRow(\''.$options['objectName'].'\', \'addMoreQuizRows'.$options['objectName'].'\');">add answer choice</a></div>';
                break;
            case self::ET_DTT_ITEMS:
                ...
                foreach($options['value'] as $key=>$value){
                    ...
                    if(is_a($value, 'DSCF_DLM_TopTen_Item')){
                        ...
                        $html .= '<div>';
                        $html .= '<button style="float:right" onClick="javascript:DSCF_DLM_Admin.deleteGenericWrapperRow(\''.$rowKey.'\')">Delete Item</button>';
                        ...
                        $html .= '<button class="button" onclick="DSCF_DLM_Admin.openMediaLibrary(\''.$rowKey.'_mediaId\','.$saved_media_id.')">Open Media Library</button>';
                ...
                $html .= '<div id="addMoreDttItemRows'.$options['objectName'].'"><a href="javascript:DSCF_DLM_Admin.addDttListRow(\''.$options['objectName'].'\', \'addMoreDttItemRows'.$options['objectName'].'\');">add list item</a></div>';
                break;
            case self::ET_RADIO:
                $first = true;
            ...
            case self::ET_TEXTAREA:
                ...
                $html .= '<textarea id="'.$options['objectName'].'" name="'.$options['objectName'].'" class="dlm-textarea" '.$rows.''.$cols.''.$maxlength.'>'.$options['value'].'</textarea> <label for="'.$options['objectName'].'"></label>';
                break;
            ...
            case self::ET_MEDIA_LIBRARY:
	            ...
	            $html .= '
	            <input id="'.$options['objectName'].'_button" type="button" class="dlm-button button" value="Open Media Library" onclick="DSCF_DLM_Admin.openMediaLibrary(\''.$options['objectName'].'\','.$saved_value.')" />
            ...
            case self::ET_MEDIA_LIBRARY_MULTIPLE:
                ...
                $html .= '<ul id="'.$options['objectName'].'_preview" class="DSCF_DLM_jquery-ui-sortable">';
                if(!empty($options['value']) && is_array($options['value'])){
                    ...
                    foreach($options['value'] as $mediaId){
                        ...
                        $html .= '<li id="'.$wrapperId.'" class="dlm-ml-thumb-wrapper">';
                        $html .= '<div class="dlm-align-right"><img src="'.WPDEVHUB_CONST_DLM_URL_IMAGES.'/cancel.png" style="width:14px;" onclick="jQuery(\'#'.$wrapperId.'\').remove();" /></div>';
                ...
                $html .= '
	            <input id="'.$options['objectName'].'_button" type="button" class="dlm-button button" value="Open Media Library" onclick="DSCF_DLM_Admin.openMediaLibraryMultiple(\''.$options['objectName'].'\')" />
	            ';
                break;
            /*
            case self::ET_BUTTON_SAVE_AND_FORWARD_ADMIN:
                ...
                if(!empty($page)){
                    $slug = DSCF_DLM_Utilities::getPageUrl($page, $params);
                    $html .= '<input type="hidden" name="forward_url_admin" value="'.$slug.'" />';
                    $html .= '<input type="submit" class="dlm-button" name="submit_save_and_forward" value="Save Changes" />';
                }else{
                    DSCF_DLM_Utilities::logMessage('variable "page" not set in params for ET_BUTTON_SAVE_AND_FORWARD_ADMIN');
                }
                ...
                switch ($option['objectType']){
                    case self::OT_DATE:
                        ...
                        if(strlen(trim($enteredDate))>1){
                            $value = DSCF_DLM_StandardObjectRecord::formatIncomingDateString($requestVars[$option['objectName']]);
                        }else{
                            $value = "";
                        }
                        $processValue = true;
                    ...
                    case self::OT_BOOLEAN:
                        //DSCF_DLM_Utilities::logMessage("Inside the Editor with Object Type String");
    ...
    public static function saveEditorChanges($object, $options, $requestVars){
        ...
        DSCF_DLM_Utilities::logMessage("Inside saveEditorChanges");
        DSCF_DLM_Utilities::logMessage("Request Vars: ".print_r($requestVars, true));
        ...
        try{
            foreach($options as $option){
                ...
                if($option['title']=='ID'){
                    ...
                    DSCF_DLM_Utilities::logMessage("ObjectType Not Found in Editor: ".print_r($option, true));
                }else{
                    switch ($option['objectType']){
                        case self::OT_DATE:
                            $enteredDate = DSCF_DLM_Utilities::getFromArray($option['objectName'], $requestVars, 0);
                        ...
                        case self::OT_NUMERIC:
                            switch ($option['formType']){
                                case self::ET_MENU_STATUS:
                                    $value = DSCF_DLM_Utilities::getFromArray($option['objectName'], $requestVars, 0);
                                    if(!is_numeric($value) || empty($value)){
                                        $value=DSCF_DLM_StandardObjectRecord::STATUS_ACTIVE;
                                    }
                                    $processValue = true;
                                    break;
                                default:
                                ...
                                case self::ET_TEXT_ARRAY_ARRAY:
                                    ...
                                    for($i=0;$i<=50;$i++){
                                        ...
                                        foreach($newItem as $k=>$v){
                                            ...
                                            //DSCF_DLM_Utilities::logMessage("Field String :: ".$fieldString);
                                            if(array_key_exists($fieldString,$requestVars)){
                                                $val = $requestVars[$fieldString];
                                                if(strlen($val)>=1){
                                                    $val = str_replace("\\", "", $val);
                                                    $newlyMadeItem[$k]=$val;
                                                    //DSCF_DLM_Utilities::logMessage("Newly Made Item: ".print_r($newlyMadeItem, true));
                                                }
                                            }
                                        }
                        ...
                        case self::OT_ARRAY:
                            switch ($option['formType']){
                                ...
                                case self::ET_ITEM_ZONE_PICKER:
                                    ...
                                    DSCF_DLM_Zone::removeZonesForItem($object->id, $allZones);
                                    DSCF_DLM_Zone::addZonesForItem($object->id, $zones);
                                ...
                                case self::ET_TEXT_ADDITIONAL:
                                    ...
                                    $value = $items;
                                    //DSCF_DLM_Utilities::logMessage("ITEMS: ".print_r($items));
                                    break;
                                case self::ET_DPM_ANSWER_CHOICE_PICKER:
                                    if(is_array($option['formOptions'])){
                                        foreach($option['formOptions'] as $key=>$value){
                                            ...
                                            if(array_key_exists($fieldString, $requestVars)){
                                                ...
                                                $status = DSCF_DLM_StandardObjectRecord::STATUS_INACTIVE;
                                                if(array_key_exists($chkString,$requestVars)){
                                                    $value = $requestVars[$chkString];
                                                    if($value == 'checked' || $value=='on'){
                                                        $status = DSCF_DLM_StandardObjectRecord::STATUS_ACTIVE;
                                ...
                                case self::ET_DTT_ITEMS:
                                    ...
                                    foreach($requestVars as $name=>$value){
                                        ...
                                        if(strpos($name, $string) !== false){
                                            ...
                                            //DSCF_DLM_Utilities::logMessage("KEY FOUND: Key[$name] Value[$value]");

                                            // Get the values
                                            $title = DSCF_DLM_Utilities::getFromArray($rowKey.'_title', $requestVars);
                                            $mediaId = DSCF_DLM_Utilities::getFromArray($rowKey.'_mediaId', $requestVars);
                                            $contents = DSCF_DLM_Utilities::getFromArray($rowKey.'_contents', $requestVars);

                                            $list = new DSCF_DLM_TopTen_Item();
                                            $list->title = $title;
                                            ...
                                            //DSCF_DLM_Utilities::logMessage("Finished with an Item: ".print_r($list, true));
                        ...
                        case self::OT_STRING:
                            switch ($option['formType']){
                                case self::ET_PASSWORD:
                                    $password = DSCF_DLM_Utilities::getFromArray($option['objectName'], $requestVars, "");
                                    ...
                                    break;
                                default:
                                    $value = DSCF_DLM_Utilities::getFromArray($option['objectName'], $requestVars, "");
                                    $value = str_replace("\\", "", $value);
                                    $value = trim($value);
                                    $processValue = true;
                                    //DSCF_DLM_Utilities::logMessage("Common Save Editor TEXT: Value:".print_r($value,true));
                                    break;
                            }
                            break;
                        case self::OT_BOOLEAN:
                            ...
                            if(array_key_exists($option['objectName'], $requestVars)){
                                ...
                                if($value=='checked' || $value=='selected' || $value=='on' || $value=='1'){
                                    DSCF_DLM_Utilities::logMessage("Settings Save: Array Key Exists Value is On");
                                    $value = true;
                                }else{
                                    DSCF_DLM_Utilities::logMessage("Settings Save: Array Key Exists Value is OFF");
                                    $value = false;
                                    $processValue = true;
                                }
                            }else{
                                //Then it means it was a checkmark and was not selected...  will not be included in responseVars
                                DSCF_DLM_Utilities::logMessage("Settings Save: Array Key Does Not Exist");
                                $value = false;
                                $processValue = true;
                            }
                            break;
            ...
            $object->save();
            DSCF_DLM_Utilities::addUserMessage('Save Successful');
    ...
    public static function renderAjaxInsertTodaysDate($formIds){
        ...
        foreach($formIds as $formId){
            $html .= 'jQuery(\'#'.$formId.'\').val(\''.DSCF_DLM_StandardObjectRecord::formatJQueryDate(time()).'\');return false;';
        }
