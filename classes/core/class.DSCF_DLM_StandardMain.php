...
class DSCF_DLM_StandardMain{
    ...
    public static function wpActionInit(){
        ...
        if(empty(static::$classname)){
            DSCF_DLM_Utilities::logError("Static DSCF_DLM_StandardMain::classname is empty and must be set");
        }
        ...
        // Check for a Plugin Upgrade
        DSCF_DLM_Utilities::checkForUpgrade(static::CURRENT_VERSION, array(static::$classname,'upgradePlugin'));

        // Register the settings
        if(static::SWITCH_USE_SETTINGS_OBJECT){
            DSCF_DLM_StandardSetting::initSettings(static::buildSettingsEditorOptions());
        }

        // Make sure the PHP session has been started if not yet already
        DSCF_DLM_Utilities::startSession();

    }

    public static function isPluginEnabled(){
        $isEnabled = DSCF_DLM_StandardSetting::getSetting(static::SETTINGS_PLUGIN_ENABLED);
        if(empty($isEnabled)){
            return false;
        }
        return true;
    ...
    public static function getSlugBase(){
        $slug = DSCF_DLM_StandardSetting::getSetting(static::SETTINGS_SLUG_BASE);
        if(empty($slug)){
    ...
    */
    public static function wpActionEnqueueScripts(){
        ...
        if(static::SWITCH_ENQUEUE_PUBLIC_RESOURCES_MAIN_CSS){
            wp_enqueue_style( WPDEVHUB_CONST_DLM_SLUG.'-main-css', WPDEVHUB_CONST_DLM_URL.'/css/wpdevhub-main.css' );
        }
    ...
    */
    public static function wpActionAdminEnqueueScripts(){
        ...
        $dimbalVars = array(
            'slug' => WPDEVHUB_CONST_DLM_SLUG,
            'url' => WPDEVHUB_CONST_DLM_URL,
        ...
        if(static::SWITCH_ENQUEUE_ADMIN_RESOURCES_ADMIN_JS){
            wp_enqueue_script( WPDEVHUB_CONST_DLM_SLUG.'-admin-js', WPDEVHUB_CONST_DLM_URL.'/js/wpdevhub-admin.js', array( 'jquery' ) );
            wp_localize_script( WPDEVHUB_CONST_DLM_SLUG.'-admin-js', 'DSCF_DLM_vars', $dimbalVars );
        }
    ...
    public static function installDatabase(){
        ...
        add_option( DSCF_DLM_Utilities::buildDatabaseVersionString(), static::CURRENT_VERSION );
    ...
    public static function addAjaxMapping($name, $callable, $type){
        ...
        switch($type){
            case self::AMT_PUBLIC:
                ...
                //DSCF_DLM_Utilities::logMessage("Public Mapping Added.  Name[".$name."] Callable[".static::$classname."][ajaxProcessHandlerWrapper] ");
                //break;
            case self::AMT_ADMIN:
                static::$ajax_admin_mappings[$name]=$callable;
                add_action( 'wp_ajax_'.$name, array( static::$classname , 'ajaxProcessHandlerWrapper' ) );
                //DSCF_DLM_Utilities::logMessage("Private Mapping Added.  Name[".$name."] Callable[".static::$classname."][ajaxProcessHandlerWrapper] ");
                break;
        }
    }
    ...
    public static function ajaxProcessHandlerWrapper(){
        //DSCF_DLM_Utilities::logMessage("Inside ajaxProcessHandlerWrapper: ".print_r(static::ajaxGetAllHandlerMappings(), true));
        DSCF_DLM_Utilities::ajaxProcessHandler(static::ajaxGetAllHandlerMappings());
    }
    ...
    public static function addCustomPostType($cpt_key, $classname){
        ...
        add_filter( 'the_content', array($classname, 'wpFilterTheContent'), 9); // Put before 10 so that we grab content before applying filters
        add_filter( 'the_excerpt', array($classname, 'wpFilterTheExcerpt'), 9);
        add_filter( 'get_the_excerpt', array($classname, 'wpFilterTheExcerpt'), 9);
    ...
    */
    public static function redirectHome(){
        DSCF_DLM_Utilities::redirect(self::getBaseUrl());
    }
    ...
    /*
     * Migrates the plugin from one version to another.  Is triggered by the CURRENT_VERSION being different
     */
    public static function upgradePlugin(){

        // First call the install database routine and update the scheme as needed
        call_user_func(array(static::$classname, 'installDatabase'));

        // get the current installed version
        $oldVersion = DSCF_DLM_Utilities::getInstalledVersionNumber();

        // Now loop through and migrate the plugin one by one.  That way if installed version is 2 and new version is 5, this will be called 3 times.
        for($i = $oldVersion; $i <= static::CURRENT_VERSION; $i++){
            static::migratePluginVersions($i);
        }

    }

    // Child plugins will implement this to indicate routines that need to be executed when migrating off an old plugin version
    public static function migratePluginVersions($oldVersion){}
