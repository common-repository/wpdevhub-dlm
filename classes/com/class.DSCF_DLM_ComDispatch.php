...
class DSCF_DLM_ComDispatch{
    ...
    public static function getEndpoint(){
        return WPDEVHUB_CONST_DLM_WDH_SERVICES_BASE_URL."/client/cp/";
    }
    ...
    public static function sendQueue(){
        ...
        DSCF_DLM_Utilities::logMessage("About to call wp_safe_remote_post with the following values URL[".self::getEndpoint()."] PARAMS: ".print_r($requestParams, true));

        $response = wp_safe_remote_post(self::getEndpoint(), $requestParams);

        //DSCF_DLM_Utilities::logMessage("Response From Dimbal Request: ".print_r($response, true));

        $dataBody = DSCF_DLM_Utilities::getFromArray('body', $response);

        //DSCF_DLM_Utilities::logMessage("Response BODY From Dimbal Request: ".print_r($data, true));

        $dataBody = json_decode($dataBody, true);

        DSCF_DLM_Utilities::logMessage("Response BODY From Dimbal Request AFTER JSON DECODING: ".print_r($dataBody, true));
        ...
        if(is_array($dataBody) && !empty($dataBody)){
            ...
            foreach($dataBody as $responseItem){
                ...
                DSCF_DLM_Utilities::logMessage("Inside ResponseItem Loop: ResponseItem:" .print_r($responseItem, true));

                // Make sure the response object had a valid ID
                $requestId = DSCF_DLM_Utilities::getFromArray("rid", $responseItem);
                if(!empty($requestId)){

                    // Get the request object out of the pending queue data out
                    $requestObject = DSCF_DLM_Utilities::getFromArray($requestId, $requestData);
