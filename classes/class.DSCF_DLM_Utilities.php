 ...
 */
class DSCF_DLM_Utilities{

    const USER_MESSAGES_KEY = "WPDEVHUB_CONST_DLM_USER_MESSAGES_COOKIE";
    ...
    public static function buildDatabaseVersionString(){
        return WPDEVHUB_CONST_DLM_SLUG . '-db-version';
    }
     ...
     */
    public static function checkForUpgrade($currentVersion, $upgradeCallback){
        global $wpdb;
        $installedVersion = DSCF_DLM_Utilities::getInstalledVersionNumber();
        if ( $currentVersion != $installedVersion) {
            // Run items in the upgrade callback (should be "upgradePlugin")
            call_user_func($upgradeCallback);
    ...
    public static function isPluginEnabled(){
        return DSCF_DLM_StandardSetting::getSetting('plugin_enabled');
    }
    ...
    public static function isPluginPro(){
        if(strtolower(WPDEVHUB_CONST_DLM_LEVEL) == 'pro'){
            return true;
        }
        return false;
    }
    ...
    public static function getBasePluginFile(){
        ...
        return WPDEVHUB_CONST_DLM_SLUG."/index.php";
    }
    ...
    public static function renderPage(){
        ...
        include WP_PLUGIN_DIR.'/'.WPDEVHUB_CONST_DLM_FOLDER.'/pages/'.$plugin_page.'.php';
    }
    ...
    public static function buildPageSlug($page){
        return WPDEVHUB_CONST_DLM_SLUG.'-'.$page;
    }
    ...
    public static function ajaxProcessHandler($handlers){
        ...
        DSCF_DLM_Utilities::logMessage("inside ajaxProcessHandler");

        // Route the call as appropriate
        $response = '';
        $action = DSCF_DLM_Utilities::getRequestVarIfExists('action');
    ...
    public static function buildAppClassName($className){
        return $className;
        //$newClassName = $className . '_' . strtoupper(WPDEVHUB_CONST_DLM_APP_CODE) . '_' . strtoupper(WPDEVHUB_CONST_DLM_PLUGIN_LEVEL);
        //return $newClassName;
    }
    ...
    public static function buildHeader($options=array()){
        ...
        $html .= '<div id="dlm-wrapper" class="dlm-wrapper">';   // Wrapper for all our pages
    ...
    public static function buildStandardObjectPage($objectClassname){
        ...
        ///////////////////////  Editor DISPLAY  ///////////////////////////
        echo DSCF_DLM_StandardEditor::buildPageTemplate($objectClassname);
    ...
    public static function logMessage($msg){
        error_log("[".WPDEVHUB_CONST_DLM_APP_CODE."] ".$msg);
    }
    ...
    public static function displayAllUserMessages(){
        ...
        $html = '';
        if(!empty($messages)){
            $html .= '<div id="dlm-user-messages-tmp" style="display:none;">';
            foreach($messages as $message){
                $rand = rand(100000,999999);
                $html .= '<div class="dlm-user-message" id="dimbal_user_message_'.$rand.'" onclick="dimbalUserMessages.remove('.$rand.')">'.$message.'</div>';
            }
            $html .= '</div>';
        }
    ...
    public static function convertMixedColorValueToRgb($color){
        ...
        $newValue = DSCF_DLM_Utilities::colorNameToHex($color, false);
        if(!empty($newValue)){
        ...
        $newValue = DSCF_DLM_Utilities::hex2RGB($color);
        if(empty($newValue)){
    ...
    public static function ensureIsBoolean($booleanToTest){
        //DSCF_DLM_Utilities::logMessage("Bool To test [$booleanToTest]");
        if(empty($booleanToTest)){
            //DSCF_DLM_Utilities::logMessage("Returning FALSE - 1");
            return false;
        }elseif($booleanToTest === 0){
            //DSCF_DLM_Utilities::logMessage("Returning FALSE - 2");
            return false;
        }elseif($booleanToTest === "0"){
            //DSCF_DLM_Utilities::logMessage("Returning FALSE - 3");
            return false;
        }elseif(strtolower($booleanToTest) === "false"){
            //DSCF_DLM_Utilities::logMessage("Returning FALSE - 4");
            return false;
        }
        //DSCF_DLM_Utilities::logMessage("Returning TRUE");
        return true;
    }
    ...
    public static function downloadTempImageFromUrl($url, $filename=""){
        ...
        while(is_file($upload_file)){
            ...
            if($counter > $maxAttempts){
                ...
                DSCF_DLM_Utilities::logError(__CLASS__."::".__FUNCTION__." - Could not create a unique filename - exiting routine. Filename[$filename]");
                return;
            }
            $upload_file = $upload_dir['path'] . '/' . $filename;
        }
    ...
    public static function getThumbnailHtmlForMediaId($mediaId, $classes="", $styles=array()){
        ...
        $styleString = DSCF_DLM_Utilities::buildStyleStringFromArray($styles);
        if(!empty($src)){
    ...
    public static function extractCommonWords($string, $stop_words, $max_count = 5) {
        ...
        arsort($frequency);
        //DSCF_DLM_Utilities::logMessage("FREQUENCY ARRAY SORTED: ".print_r($frequency,true));

        //Now just provide the keys... although this is now sorted
        $frequency = array_keys($frequency);

        //DSCF_DLM_Utilities::logMessage("FREQUENCY ARRAY KEYS: ".print_r($frequency,true));
    ...
    public static function ajaxValidateSlugHandler(){
        ...
        $classname = DSCF_DLM_Utilities::getRequestVarIfExists("class_name");
        $slugText = DSCF_DLM_Utilities::getRequestVarIfExists("slug_text");
        $elemId = DSCF_DLM_Utilities::getRequestVarIfExists("elem_id");
        $objectId = DSCF_DLM_Utilities::getRequestVarIfExists("object_id");
        ...
        if(!empty($objectId)){
            ...
            if(!empty($object)){
                $slugTest = DSCF_DLM_Utilities::buildSlug($slugText);
    ...
    public static function getFormattedBasicAlignmentString($id){
        ...
        return DSCF_DLM_Utilities::getFromArray($id, $collection, '');
    }
    ...
    public static function migrateTableNameIfNeeded($oldTableName, $newTableName){
        ...
        global $wpdb;
        $migrationKey = "dlm_tbn_migr_f_".$oldTableName."_t_".$newTableName;

        $value = DSCF_DLM_StandardSetting::getSetting($migrationKey);
        if(!empty($value)){
        ...
        DSCF_DLM_StandardSetting::saveSetting($migrationKey, true);
    ...
    public static function migratePostTypesIfNeeded($oldType, $newType){
        ...
        global $wpdb;
        $migrationKey = "dlm_cpt_migr_f_".$oldType."_t_".$newType;
    ...
    public static function migrateTaxonomyTypesIfNeeded($oldType, $newType){
        global $wpdb;
        $migrationKey = "dlm_tax_migr_f_".$newType."_t_".$oldType;
        ...
        DSCF_DLM_Utilities::logMessage($sql);
        $wpdb->query($sql);
    ...
    public static function migrateMetaBoxDataIfNeeded($oldType, $newType){
        global $wpdb;
        $migrationKey = "dlm_cptm_migr_f_".$newType."_t_".$oldType;
