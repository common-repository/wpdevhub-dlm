...
class DSCF_DLM_VirtualPages_Controller implements DSCF_DLM_VirtualPages_ControllerInterface{
    ...
    function __construct(DSCF_DLM_VirtualPages_TemplateLoaderInterface $loader){
    ...
    function init() {
        do_action( WPDEVHUB_CONST_DLM_SLUG.'_virtual_pages', $this );
    }

    function addPage(DSCF_DLM_VirtualPages_PageInterface $page){
    ...
    function dispatch( $bool, \WP $wp ) {
        ...
        if ($this->checkRequest() && $this->matched instanceof DSCF_DLM_VirtualPages_Page) {
    ...
    private function checkRequest() {
        ...
        while( $this->pages->valid() ) {
            ...
            //DSCF_DLM_Utilities::logMessage("checkRequest: Path[$path] SetupUrl[".$currentPageUrl."]");
            ...
            if(strpos($currentPageUrl, self::PATH_SEPERATOR) !== FALSE){
                ...
                //DSCF_DLM_Utilities::logMessage("Inside the Path Seperator");
                //DSCF_DLM_Utilities::logMessage("Paths: ".print_r($pathParts, true));
                //DSCF_DLM_Utilities::logMessage("Current: ".print_r($currentParts, true));
                ...
                foreach($currentParts as $key=>$currentPart){
                    ...
                    //DSCF_DLM_Utilities::logMessage("Path Part test: CurrentPart[$currentPart] PathPart[$pathPart]");
                    ...
                    if(strpos($currentPart, self::PATH_SEPERATOR) !== FALSE){
                        ...
                        //DSCF_DLM_Utilities::logMessage("Path Part test: Saved as a Variable");
                    }elseif($currentPart === $pathPart){
                        // String match - so far so good
                        //DSCF_DLM_Utilities::logMessage("Path Part test: String Match");
                    }else{
                        // No string match and not variable -- bad match - break out of the foreach loop
                        //DSCF_DLM_Utilities::logMessage("Path Part test: No String Match and No Variable - break out.");
                        $this->pages->next();
                        continue 2;
                    }
                }
                ...
                //DSCF_DLM_Utilities::logMessage("A match was found inside the path seperators: ".print_r($this, true));
